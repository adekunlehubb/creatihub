...
async function geminiImage(order) {
  ...
  for (const v of variants) {
    ...
    try {
      ...
      if (v === 'primary') {
        const brief = await geminiImageConceptBrief(order);
        out.push({
          id: shortId('txt'),
          kind: 'text',
          ...
          encoding: 'utf8',
          isDemo: false,
          summary: `Visual concept brief for ${order.serviceName} (image gen unavailable on free tier — includes ready-to-use AI prompt)`,
          provider: 'gemini-concept',
          generatedAt: new Date().toISOString()
        });
      }
